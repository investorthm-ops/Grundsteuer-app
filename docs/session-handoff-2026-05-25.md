...
- Nutzer koennen eingeladen werden.
...
- Supabase Auth Redirect URLs sind fuer Produktion und Localhost gesetzt.
- Supabase E-Mail-Templates fuer Invite und Reset sind deutsch und nutzen `{{ .ConfirmationURL }}`.
...
- Abgelaufene oder ungueltige Passwort-Links zeigen jetzt eine bessere Nutzerhilfe.

## Kritische offene Punkte

1. **SMTP fuer Auth-Mails**
   - Supabase Default-Mailer ist fuer Pilotkunden zu wackelig.
   - Spaeter eigenen SMTP-Anbieter einrichten, z. B. Brevo, Resend, Mailgun, Postmark oder SendGrid.
   - Erwartete MVP-Kosten grob: 0 bis 20 EUR pro Monat.

2. **MAB pausiert**
   - Nutzer existiert in Supabase: `djibril.brand@gmail.com`
   - Organisation `Testkunde MAB` nicht loeschen.
...
2. Live-Deployment nach letztem Push pruefen.
3. Entscheiden, ob PROJ-14 werden soll:
   - Option A: Datenqualitaets-Dashboard fuer Admins
...
Meine Empfehlung: **PROJ-14 Pilotkunden-Onboarding und Datenqualitaet**. SMTP bleibt als Produktionspunkt wichtig, muss aber erst entschieden werden, wenn ein Mailanbieter ausgewaehlt ist.
